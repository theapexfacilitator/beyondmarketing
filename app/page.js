    ...
    <div>
      ...
      <section className="container mx-auto px-4 py-16">
        <div className="text-center max-w-2xl mx-auto mb-10">
          <div className="text-sm text-blue-400 font-medium mb-2">Packages</div>
          <h2 className="text-3xl md:text-4xl font-semibold tracking-tight">Pick the level of partnership that fits.</h2>
          <p className="mt-3 text-muted-foreground">Five packages built around the same connected framework — from local SEO to a full outsourced growth department.</p>
        </div>
        <div className="grid md:grid-cols-2 lg:grid-cols-5 gap-4">
          {PACKAGES.map(p => <PackageCard key={p.n} p={p} go={go} size="compact" />)}
        </div>
        <div className="text-center mt-8">
          <Button variant="outline" onClick={() => go('pricing')}>See full pricing details <ArrowRight className="w-4 h-4 ml-2" /></Button>
        </div>
      </section>
...
const PACKAGES = [
  ...
  {
    n: 'Blueprint', price: '$1,500', p: 'month', tag: 'Strategy first',
...
function PackageCard({ p, go, size = 'default' }) {
  const compact = size === 'compact'
  return (
    <Card className={`relative ${p.featured ? 'border-blue-500/50 shadow-2xl shadow-blue-500/10 md:scale-[1.02]' : 'border-border/60'} bg-secondary/30`}>
      ...
      <CardHeader className={compact ? 'pb-3' : ''}>
        ...
        <CardTitle className={compact ? 'text-xl' : 'text-2xl'}>{p.n}</CardTitle>
        {p.desc && <CardDescription className="text-xs">{p.desc}</CardDescription>}
        <div className={`${compact ? 'text-2xl' : 'text-3xl'} font-semibold mt-2`}>{p.price}<span className="text-sm text-muted-foreground font-normal">{p.p ? ' / ' + p.p : ''}</span></div>
      </CardHeader>
      <CardContent>
  ...
  return (
    <div>
      ...
      <section className="container mx-auto px-4 py-8 grid md:grid-cols-2 lg:grid-cols-5 gap-4">
        {PACKAGES.map(p => <PackageCard key={p.n} p={p} go={go} />)}
      </section>
      <FinalCTA go={go} />
    </div>
  )
